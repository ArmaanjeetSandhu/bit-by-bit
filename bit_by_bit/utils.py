...
class TorrentFile:
    def __init__(self, file_path):
        with open(file_path, "rb") as f:
            ...
            self.torrent: Dict[str, Any] = cast(
                Dict[str, Any], decode_bencode(bencoded_string)
            )
        self.info_dict: Dict[str, Any] = cast(Dict[str, Any], self.torrent["info"])
        self.piece_length = int(self.info_dict["piece length"])
        self.file_length = int(self.info_dict["length"])
        ...
        self.piece_hashes = get_piece_hashes(cast(str, self.info_dict["pieces"]))
        self.total_pieces = len(self.piece_hashes)

    @property
    def tracker_url(self) -> str:
        return cast(str, self.torrent["announce"])

    def get_piece_size(self, piece_index):
        if piece_index == self.total_pieces - 1:
            return int(self.file_length) - (int(self.total_pieces) - 1) * int(
                self.piece_length
            )
        return int(self.piece_length)
...
class PeerConnection:
    def __init__(self, peer_addr, info_hash):
        ...
        self.extension_ids: Dict[str, int] = {}
        self.metadata = None
    ...
    def connect(self, support_extensions=False, handshake_only=False):
        ...
        try:
            ...
            if handshake_only:
                self.connected = True
                return True
            _, message_id, _ = self._receive_message()
            if message_id != 5:
                raise ValueError(
    ...
    def perform_handshake(self, support_extensions=False):
        return self._perform_handshake(support_extensions)
    ...
    def check_peer_supports_extensions(self, handshake_response):
        return self._check_peer_supports_extensions(handshake_response)
    ...
    def receive_extension_handshake(self):
        self._receive_extension_handshake()
    ...
    def process_extension_message(self, payload):
        self._process_extension_message(payload)
    ...
    def _process_extension_message(self, payload):
        ...
        if ext_msg_id == 0:
            ...
            handshake_dict: Dict[str, Any] = cast(
                Dict[str, Any], decode_bencode(ext_payload_str)
            )
            if "m" in handshake_dict:
                self.extension_ids = cast(Dict[str, int], handshake_dict["m"])
...
class Tracker:
    ...
    def get_peers(self, info_hash, file_length, peer_id=None):
        ...
        decoded_response: Dict[str, Any] = cast(
            Dict[str, Any], decode_bencode(response_string)
        )
        peers = cast(str, decoded_response["peers"])
...
def download_file(torrent_file, output_file):
    ...
    with open(output_file, "wb") as f:
        f.truncate(int(torrent.file_length))
    tracker = Tracker(torrent.tracker_url)
    peers = tracker.get_peers(torrent.info_hash_bytes, torrent.file_length)
    if not peers:
        ...
        try:
            ...
            for piece_index in range(torrent.total_pieces):
                ...
                with open(output_file, "r+b") as f:
                    f.seek(piece_index * int(torrent.piece_length))
                    f.write(piece_data)
                print(
        ...
        try:
            ...
            for peer_addr in peers:
                try:
                    ...
                    handshake_response = peer.perform_handshake(support_extensions=True)
                    peer.extensions_enabled = True
                    peer.peer_supports_extensions = peer.check_peer_supports_extensions(
                        handshake_response
                    )
                    if not peer.peer_supports_extensions:
                        peer.close()
                        continue
                    _, message_id, _ = peer.receive_message()
                    peer.send_extension_handshake()
                    found_extension = False
                    for _ in range(3):
                        try:
                            _, message_id, payload = peer.receive_message()
                            if message_id == 20:
                                peer.process_extension_message(payload)
